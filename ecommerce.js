...
function searchFunc(){
    ...
    mappedData = myProducts.products.map(meriProduct => {
        var currentval = meriProduct.title.toLowerCase().search(searchTerm);
        console.log(currentval)
        if(currentval>=0){
            dropDown.innerHTML += `<div>${meriProduct.title}</div><br>`;
        }
    })
    if (this.value == ""){
            dropDown.innerHTML = "";
